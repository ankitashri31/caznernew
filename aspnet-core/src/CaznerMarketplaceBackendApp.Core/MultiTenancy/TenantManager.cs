...
using CaznerMarketplaceBackendApp.MultiTenancy.Demo;
using Abp.Extensions;
using Abp.Notifications;
using Abp.Runtime.Security;
using Microsoft.AspNetCore.Identity;
using CaznerMarketplaceBackendApp.Notifications;
using System;
...
namespace CaznerMarketplaceBackendApp.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
        ...
        private readonly IRepository<UserSubscriptionPlan, long> _userSubscriptionRepository;
        ...
        public TenantManager(
            ...
            IRepository<UserSubscriptionPlan, long> userSubscriptionRepository,
            IRepository<UserDetails, long> userDetailsRepository,
            IRepository<Role, int> rolesRepository) : base(
                tenantRepository,
            ...
            string emailActivationLink)
        {
            ...
            {
                throw new UserFriendlyException(LocalizationManager.GetString(CaznerMarketplaceBackendAppConsts.LocalizationSourceName, "TrialWithoutEndDateErrorMessage"));
            }
        ...
        public async Task<int> CreateWithAdminUserCustomAsync(
          string tenancyName,
          string name,
          string surName,
          string phoneNumber,
          ...
          long SubscriptionPlanId)
        {
            ...
            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                ...
                //grant all permissions to admin role
                // var adminRole = _roleManager.Roles.Single(r => r.Name == StaticRoleNames.Tenants.Supplier);
                // await _roleManager.GrantAllPermissionsAsync(adminRole);
                ...
                try
                {
                    ...
                    userDetails.UserId = adminUser.Id;
                    userDetails.TenantId = tenant.Id;
                    string RoleName = string.Empty;
                    using (_unitOfWorkManager.Current.SetTenantId(tenant.Id))
                    {
                        try
                        {
                            long Id = await _userDetailsRepository.InsertAndGetIdAsync(userDetails);

                            #region subscription plan

                            if (SubscriptionPlanId > 0)
                            {
                                UserSubscriptionPlan Plan = new UserSubscriptionPlan();
                                Plan.UserId = adminUser.Id;
                                Plan.SubscriptionPlanId = SubscriptionPlanId;
                                Plan.IsActive = true;
                                await _userSubscriptionRepository.InsertAsync(Plan);
                            }
                        ...
                        if (roleName.ToLower().Trim() == "supplier")
                        {
                            RoleName = "Supplier";
                        }
                        else if (roleName.ToLower().Trim() == "distributor")
                        {
                    ...
                    using (_unitOfWorkManager.Current.SetTenantId(null))
                    {
                        var adminRole = _rolesRepository.GetAll().Where(r => r.Name == RoleName).FirstOrDefault();
                        await _roleManager.GrantAllPermissionsAsync(adminRole);
                        //Assign admin user to admin role!
                        CheckErrors(await _userManager.AddToRoleAsync(adminUser, adminRole.Name));
                    }
                ...
                #region commented notification part for now
                //await _appNotifier.WelcomeToTheApplicationAsync(adminUser);
        ...
        public async Task<string> CreateDistributorUserAsync(
     int tenantId,
     string name,
     string surName,
     string phoneNumber,
     ...
     long SubscriptionPlanId)
        {
            ...
            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                ...
                //Create admin user for the tenant
                var adminUser = User.CreateTenantAdminUser(tenantId, adminEmailAddress);
                adminUser.ShouldChangePasswordOnNextLogin = shouldChangePasswordOnNextLogin;
                ...
                try
                {
                    ...
                    using (_unitOfWorkManager.Current.SetTenantId(tenantId))
                    {
                        ...
                        if (roleName.ToLower().Trim() == "distributor")
                        {
                            RoleName = "Distributor";
                        }
                    }
            ...
            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                using (_unitOfWorkManager.Current.SetTenantId(tenantId))
                {
                    await _notificationSubscriptionManager.SubscribeToAllAvailableNotificationsAsync(new UserIdentifier(tenantId, newAdminId));
                    await _unitOfWorkManager.Current.SaveChangesAsync();
                    await uow.CompleteAsync();
                }
            }
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(CaznerMarketplaceBackendAppConsts.LocalizationSourceName).GetString("FreeEditionsCannotHaveTrialVersions");
