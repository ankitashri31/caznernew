...
namespace CaznerMarketplaceBackendApp.Net.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ISingletonDependency
    {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("CaznerMarketplaceBackendApp.Net.Emailing.EmailTemplates.default.html"))
                {
