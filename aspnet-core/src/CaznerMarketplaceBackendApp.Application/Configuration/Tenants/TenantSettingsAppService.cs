...
using CaznerMarketplaceBackendApp.Configuration.Tenants.Dto;
using CaznerMarketplaceBackendApp.Security;
using CaznerMarketplaceBackendApp.Storage;
using CaznerMarketplaceBackendApp.Timing;

namespace CaznerMarketplaceBackendApp.Configuration.Tenants
{
    ...
    public class TenantSettingsAppService : SettingsAppServiceBase, ITenantSettingsAppService
    {
        ...
        private async Task<SecuritySettingsEditDto> GetSecuritySettingsAsync()
        {
            var passwordComplexitySetting = new PasswordComplexitySetting
            {
                ...
                RequiredLength = await SettingManager.GetSettingValueAsync<int>(AbpZeroSettingNames.UserManagement.PasswordComplexity.RequiredLength) == 0 ? 8 : await SettingManager.GetSettingValueAsync<int>(AbpZeroSettingNames.UserManagement.PasswordComplexity.RequiredLength)
            };
        ...
        {
            if (_multiTenancyConfig.IsEnabled && !CaznerMarketplaceBackendAppConsts.AllowTenantsToChangeEmailSettings)
            {
                return;
            }
