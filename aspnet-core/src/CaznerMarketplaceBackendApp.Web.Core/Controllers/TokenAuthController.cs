...
using CaznerMarketplaceBackendApp.Web.Authentication.JwtBearer;
using CaznerMarketplaceBackendApp.Web.Authentication.TwoFactor;
using CaznerMarketplaceBackendApp.Web.Models.TokenAuth;
...
using CaznerMarketplaceBackendApp.Net.Sms;
using CaznerMarketplaceBackendApp.Notifications;
using CaznerMarketplaceBackendApp.Security.Recaptcha;
using CaznerMarketplaceBackendApp.Web.Authentication.External;
...
namespace CaznerMarketplaceBackendApp.Web.Controllers
{
    ...
    public class TokenAuthController : CaznerMarketplaceBackendAppControllerBase
    {
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            {
                string email = NormalizeKey(model.UserNameOrEmailAddress);
                var CheckTenancy = _userManager.Users.FirstOrDefault(x => x.NormalizedEmailAddress == email);
                if (CheckTenancy == null)
                {
                    throw new UserFriendlyException(LocalizationManager.GetString(CaznerMarketplaceBackendAppConsts.LocalizationSourceName, "TenantDoesNotExistsError"));

                }
                var TenantData = _tenantCache.Get(CheckTenancy.TenantId.Value);
                tenancyName = TenantData.TenancyName;
            }
